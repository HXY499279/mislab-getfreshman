import React from 'react'
// 引入外部资源
import './index.css'
import liucheng1 from '../../../assets/liucheng1.png'
import liucheng2 from '../../../assets/liucheng1.png'
import liucheng3 from '../../../assets/liucheng1.png'

export default function index() {
    return (
        <div className="prc-bgc">
            <div className="prc-wrap">
                <div className="prc-top">
                    <div className="prc-top-left prc-top-process">
                        <img
                            src=""
                            alt="图片"
                            className="prc-top-left-img"
                        />
                        <div className="prc-top-left-h">
                            <span className="prc-top-left-h1">
                                报名/预习
                            </span>
                            <span className="prc-top-left-h2">
                                8月中旬—9月中旬
                            </span>
                        </div>
                    </div>
                    <div className="prc-top-icon">
                        <i className="iconfont prc-top-icon-i"></i>
                    </div>
                    <div className="prc-top-middle prc-top-process">
                        <img
                            src=""
                            alt="图片"
                            className="prc-top-middle-img"
                        />
                        <div className="prc-top-middle-h">
                            <span className="prc-top-middle-h1">
                                招新/培训
                            </span>
                            <span className="prc-top-middle-h2">
                                9月中旬—12月中旬
                            </span>
                        </div>
                    </div>
                    <div className="prc-top-icon">
                        <i className="iconfont prc-top-icon-i"></i>
                    </div>
                    <div className="prc-top-right prc-top-process">
                        <img
                            src=""
                            alt="图片"
                            className="prc-top-right-img"
                        />
                        <div className="prc-top-right-h">
                            <span className="prc-top-right-h1">
                                offer/项目
                            </span>
                            <span className="prc-top-right-h2">
                                12月中旬—长期
                            </span>
                        </div>
                    </div>
                </div>
                <div className="prc-middle-1">
                    <img
                        src=""
                        alt="图片"
                        className="prc-middle-1-img"
                    />
                </div>
                <div className="prc-middle-2">
                    <div className="prc-middle-2-title">
                        想和优秀的你&nbsp;&nbsp;&nbsp;&nbsp;一起改变世界
                    </div>
                </div>
                <div className="prc-bottom">
                    <div className="prc-bottom-left">
                        <span className="prc-bottom-text">报名投递</span>
                    </div>
                    <div className="prc-bottom-middle">
                        <span className="prc-bottom-text">学员系统</span>
                    </div>
                    <div className="prc-bottom-right">
                        <span className="prc-bottom-text">招新咨询</span>
                    </div>
                </div>
            </div>
        </div>
    )
}
